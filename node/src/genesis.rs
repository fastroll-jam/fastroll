use rjam_block::types::block::{BlockHeader, EpochMarker, EpochMarkerValidatorKey};
use rjam_common::{BandersnatchPubKey, Ed25519PubKey, Hash32};

pub(crate) fn genesis_block_header() -> Result<BlockHeader, ()> {
    Ok(BlockHeader {
        epoch_marker: Some(EpochMarker {
            entropy: Hash32::default(),
            tickets_entropy: Hash32::default(),
            validators: Box::new([
                EpochMarkerValidatorKey {
                    bandersnatch_key: BandersnatchPubKey::try_from_hex(
                        "0x5e465beb01dbafe160ce8216047f2155dd0569f058afd52dcea601025a8d161d",
                    )?,
                    ed25519_key: Ed25519PubKey::try_from_hex(
        ...
        block_author_index: u16::MAX, // 65535
        ..Default::default()
    })
}
