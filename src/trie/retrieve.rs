...
impl<'a> StateRetriever<'a> {
    pub(crate) fn new(db: &'a dyn KeyValueDB) -> Self {
        Self { db }
    }
    ...
    fn retrieve_state_component(&self, merkle_path_hash: Hash32) -> Octets {
    ...
    pub fn get_authorization_pool(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Alpha))
    }

    pub fn get_authorization_queue(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Phi))
    }

    pub fn get_block_history(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Beta))
    }

    pub fn get_safrole_state(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Gamma))
    }

    pub fn get_verdicts(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Psi))
    }

    pub fn get_entropy_accumulator(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Eta))
    }

    pub fn get_staging_validator_set(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Iota))
    }

    pub fn get_active_validator_set(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Kappa))
    }

    pub fn get_past_validator_set(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Lambda))
    }

    pub fn get_pending_reports(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Rho))
    }

    pub fn get_recent_timeslot(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Tau))
    }

    pub fn get_privileged_services(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Chi))
    }

    pub fn get_validator_statistics(&self) -> Octets {
        self.retrieve_state_component(construct_key(M::Pi))
    }

    pub fn get_service_storage(&self, service_idx: u32, storage_key: &Hash32) -> Octets {
        self.retrieve_state_component(construct_key_with_service_and_hash(
            service_idx,
            storage_key,
        ))
    }
    ...
    pub fn get_service_lookup(&self, service_idx: u32, lookup_key: &(Hash32, u32)) -> Octets {
        ...
        self.retrieve_state_component(construct_key_with_service_and_data(
            service_idx,
            &encoded_lookup_key,
        ))
    }
