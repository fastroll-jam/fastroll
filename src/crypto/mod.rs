...
fn validator_set_to_ring<S: Suite>(validator_set: ValidatorSet) -> Result<Vec<Public<S>>, ()> {
...
pub(crate) fn generate_ring_root(validator_set: ValidatorSet) -> Result<RingCommitment, ()> {
    let ring = validator_set_to_ring::<BandersnatchSha512Ell2>(validator_set)?;
    let verifier = Verifier::new(ring);
    Ok(verifier.commitment)
}

pub(crate) fn generate_ring_root_serialized(validator_set: ValidatorSet) -> Result<String, ()> {
    let commitment = generate_ring_root(validator_set)?;
