use crate::{
    ...
    IS_AUTHORIZED_GAS_PER_WORK_PACKAGE, MAX_AUTH_POOL_SIZE, MAX_EXPORTS_PER_PACKAGE,
    MAX_EXTRINSICS_PER_PACKAGE, MAX_IMPORTS_PER_PACKAGE, MAX_IS_AUTHORIZED_CODE_SIZE,
    MAX_LOOKUP_ANCHOR_AGE, MAX_PACKAGE_AND_DATA_SIZE, MAX_REPORT_DEPENDENCIES,
    MAX_SERVICE_CODE_SIZE, MAX_TICKETS_PER_EXTRINSIC, MAX_WORK_ITEMS_PER_PACKAGE,
    MIN_BALANCE_PER_ITEM, MIN_BALANCE_PER_OCTET, MIN_BASIC_BALANCE, PENDING_REPORT_TIMEOUT,
    PREIMAGE_EXPIRATION_PERIOD, REFINE_GAS_PER_WORK_PACKAGE, SLOT_DURATION, TICKETS_PER_VALIDATOR,
...
pub fn encode_constants_for_fetch_hostcall() -> Result<Vec<u8>, JamCodecError> {
    ...
    MAX_TICKETS_PER_EXTRINSIC.encode_to_fixed(&mut buf, 2)?; // K
    MAX_LOOKUP_ANCHOR_AGE.encode_to_fixed(&mut buf, 4)?; // L
    TICKETS_PER_VALIDATOR.encode_to_fixed(&mut buf, 2)?; // N
