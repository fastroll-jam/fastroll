...
macro_rules! continue_with_reg_write {
    ($curr_pc:expr, $program_state:expr, $reg_idx:expr, $reg_val:expr) => {
        Ok(SingleStepResult {
            exit_reason: ExitReason::Continue,
            state_change: VMStateChange {
                new_pc: Interpreter::next_pc($curr_pc, $program_state),
...
macro_rules! continue_with_mem_write {
    ($curr_pc:expr, $program_state:expr, $offset:expr, $data:expr) => {
        Ok(SingleStepResult {
            exit_reason: ExitReason::Continue,
            state_change: VMStateChange {
